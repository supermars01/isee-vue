  ...
  <div class="module">
    ...
    <el-container>
      <el-aside class="hidden-md-and-down">
        <el-autocomplete class="inline-input" placeholder="请输入名称" prefix-icon="el-icon-search" :fetch-suggestions="querySearch" v-model="name" @select="handleSelect"></el-autocomplete>
        <el-table
          ref="listTable"
          :data="nameList"
...
export default {
  ...
  created(){
    ...
    localStorage.getItem("dataset") && this.$store.replaceState(Object.assign(this.$store.state,JSON.parse(localStorage.getItem("dataset"))));
  ...
  methods: {
    ...
    handleCurrentChange(val) {
        this.currentRow = val
        this.currentData = this.dataList[this.currentRow.id-1]
        this.drawRadar();
    },
    handleSelect(val){
    ...
    querySearch(queryString,cb){
      var results = queryString ? this.nameList.filter(item=>item.value.indexOf(queryString) === 0):this.nameList
      cb(results)
    },
    drawRadar() {
